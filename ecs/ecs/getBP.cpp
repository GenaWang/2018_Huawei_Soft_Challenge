...
void BpNet::train(vector<vector<double> > &p,vector<vector<double> > &t)  
{  
...
void getBP(vector<double> &x,vector<double> &y,vector<double> &yh,time_t begin,time_t end)
{  
    double max = 0.0;  
    BpNet bp;  
    bp.init();  
    trainsample = x.size();
    vector<vector<double> > X(trainsample);
    vector<vector<double> > Y(trainsample);
    ...
    {
    	X[i].push_back(int(x[i]));
	Y[i].push_back(y[i]/max);
    }
    int times=0;  
    while(times<9000)
    {  
